...
if [ ! -f "${APP_DIR}/server.js" ]; then
  echo "Frontend standalone server not found: ${APP_DIR}/server.js"
  exit 1
fi

if [ ! -f "${ENV_FILE}" ]; then
  echo "Environment file not found: ${ENV_FILE}"
  echo "Create it at ${ENV_FILE} before starting the frontend."
  exit 1
fi
...
echo "Starting frontend on ${FRONTEND_HOSTNAME}:${FRONTEND_PORT}..."
cd "${APP_DIR}"
